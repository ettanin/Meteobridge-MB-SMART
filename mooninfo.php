...
include('settings.php');include('livedata.php');
...
<style>
@font-face{font-family:weathertext;src:url(css/fonts/verbatim-regular.woff) format("woff"),url(fonts/verbatim-regular.woff2) format("woff2"),url(fonts/verbatim-regular.ttf) format("truetype")}body,html{font-size:13px;font-family:weathertext,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,2fr));grid-gap:5px;align-items:stretch;color:#f5f7fc;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}.grid>article{border:1px solid rgba(245,247,252,.02);box-shadow:2px 2px 6px 0 rgba(0,0,0,.6);padding:5px;font-size:.75em;-webkit-border-radius:4px;border-radius:4px;background:0;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;height:160px}.grid>article img{max-width:50%;max-height:100px}a{color:#aaa;text-decoration:none}.weather34darkbrowser{position:relative;background:0;width:97%;height:30px;margin:auto;margin-top:-5px;margin-left:0;border-top-left-radius:5px;border-top-right-radius:5px;padding-top:10px}.weather34darkbrowser[url]:after{content:attr(url);color:#aaa;font-size:10px;position:absolute;left:0;right:0;top:0;padding:4px 15px;margin:11px 10px 0 auto;border-radius:3px;background:rgba(97,106,114,.3);height:20px;box-sizing:border-box}blue{color:#01a4b4}orange{color:#009bb4}orange1{position:relative;color:#009bb4;margin:0 auto;text-align:center;margin-left:5%;font-size:1.1rem}green{color:#aaa}red{color:#f37867}red6{color:#d65b4a}value{color:#fff}yellow{color:#cc0}purple{color:#916392}meteotextshowertext{font-size:1.2rem;color:#009bb4}meteorsvgicon{color:#f5f7fc}.moonphasesvg{align-items:right;justify-content:center;display:flex;max-height:120px}.moonphasetext{font-size:.8rem;color:#f5f7fc;position:absolute;display:inline;left:145px;top:100px}moonphaseriseset{font-size:.75rem}credit{position:relative;font-size:.7em;top:10%}.actualt{position:relative;left:5px;-webkit-border-radius:3px;-moz-border-radius:3px;-o-border-radius:3px;border-radius:3px;background:rgba(74,99,111,.1);padding:5px;font-family:Arial,Helvetica,sans-serif;width:100px;height:.8em;font-size:.8rem;padding-top:2px;color:#aaa;align-items:center;justify-content:center;margin-bottom:10px;top:0}.actualw{position:relative;left:5px;-webkit-border-radius:3px;-moz-border-radius:3px;-o-border-radius:3px;border-radius:3px;background:rgba(74,99,111,.1);padding:5px;font-family:Arial,Helvetica,sans-serif;width:100px;height:.8em;font-size:.8rem;padding-top:2px;color:#aaa;align-items:center;justify-content:center;margin-bottom:10px;top:0}.moonphaseing{width:110px;padding-bottom:50px;margin-top:-10px}.weather34moonphasesvg{position:absolute;left:25px;top:50px;width:120px;}
.weather34moonforeground{fill:rgba(230,232,239,.3);stroke:0;stroke-width:0;}
.weather34moonbackground{fill:rgba(86,95,103,.8);stroke:0;stroke-width:0;}
.weather34moonphasesvg{position:absolute; left:25px;}
small{font-size:8px;}
large{position:relative;top:-5px;left:60px;font-size:13px;width:120px}
.date{position:relative;top:-15px;left:60px;font-size:11px;width:180px}
</style>
...
<svg><circle cx="50" cy="50" r="49.5" fill="rgba(86,95,103,.8)"/><path id="weather34themoon" fill="rgba(230, 232, 239, .3)"/></svg>
<script>
showMoon();
function showMoon() {
	var day = Date.now() / 86400000;
	var weather34Moon = Date.UTC(2018, 0, 17, 1, 0, 0, 0);
	var refDay = weather34Moon / 86400000;
	var weather34phase = (day - refDay) % 29.530588853;
	var phaseStr;
	var s=String;	
	document.getElementById("weather34moonphase");	
	var val=15;
	var weather34moonCurve;	
	var lf=Math.min(3-4*(weather34phase/30),1);
	var lc=Math.abs(lf*50);
	var lb=(lf<0) ? "0" : "1";
	var rf=Math.min(3+4*((weather34phase-30)/30),1);
	var rc=Math.abs(rf*50);
	var rb=(rf<0) ? "0" : "1";	
	weather34moonCurve="M 50,0 "+ "a "+s(lc)+",50 0 0 "+lb+" 0,100 "+ "a "+s(rc)+",50 0 0 "+rb+" 0,-100";	
	document.getElementById("weather34themoon").setAttribute("d",weather34moonCurve);
}</script>
</article>

<div class=moonphasetext>    
<?php echo " ";{$day = date('l jS F Y');
if($day===date("l jS F Y",strtotime('2020-2-9'))){echo 'Super <blue>Moon</blue>';}
...
<svg id="i-ban" viewBox="0 0 32 32" width="10" height="10" fill="rgba(255, 136, 65, 1.00)" stroke="rgba(255, 136, 65, 1.00)" 
...
$moon = new MoonPhase();$moonage =round($moon->age(),2);echo "Cycle is <blue>", round($moonage,0)," days old";?></blue>
...
if($lunayear==2020 ){	
	...
	echo $info." Total Lunar <blue>Eclipse </blue> 8 Nov 2021<br>";	
	}
	
else if($lunayear==2022 ){
	echo $info." Partial Lunar <blue>Eclipse </blue> 30 Apr 2022 <br>	";
	echo $info." Total Lunar <blue>Eclipse </blue> 16 May 2022<br>";
	echo $info." Partial Solar <blue>Eclipse </blue> 25 Oct 2022<br>";
	echo $info." Total Lunar <blue>Eclipse </blue> 8 Nov 2022<br>";		
		}		
